...
export const registrarCheckInOuOutComLocal = async (atividadeId, latitude, longitude) => {
  const status = await verificarStatusAtividade(atividadeId);
  const dataHora = new Date().toISOString();

  await db.runAsync(`
    INSERT INTO CheckInsCheckOuts (atividade_id, data_hora_checkin, latitude, longitude)
    VALUES (?, ?, ?, ?)
  `, [atividadeId, dataHora, latitude, longitude]);
...
/////////////// historico click cvhek in

export const getHistoricoRecentes = async (limite = 10) => {
  const rows = await db.getAllAsync(`
    SELECT C.id, C.data_hora_checkin, C.atividade_id, A.apelido, A.nome
    FROM CheckInsCheckOuts C
...
export const getCheckinDetalhado = async (id) => {
  const rows = await db.getAllAsync(`
    SELECT C.*, A.nome, A.apelido
    FROM CheckInsCheckOuts C
...
///////////////////////////////todos check
export const getTodosCheckinsComAtividade = async () => {
  const rows = await db.getAllAsync(`
    SELECT C.*, A.nome, A.apelido
    FROM CheckInsCheckOuts C
    JOIN Atividades A ON A.id = C.atividade_id
